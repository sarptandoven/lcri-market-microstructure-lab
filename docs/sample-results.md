...
before treating the residual signal as robust. `lcri_gap_delta_scorecard.json`
reports mean, median, and share-level stability edges for release gates.
`lcri_gap_delta_flags.csv` adds a plain stability label to each row for quick
filtering. The matching chart shows the same comparison visually, while
`lcri_gap_delta_summary.json` records row counts and the strongest stability and
instability contexts for automated report checks.
