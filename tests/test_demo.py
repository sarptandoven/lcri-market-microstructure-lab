...
def test_run_demo_writes_reports(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert (tmp_path / "passive_fill_event_window_regime_summary.csv").exists()
    assert (tmp_path / "passive_fill_event_window_regime_scorecard.json").exists()
    ...
    assert (tmp_path / "heldout_passive_fill_event_window_regime_summary.csv").exists()
    assert (tmp_path / "heldout_passive_fill_event_window_regime_scorecard.json").exists()
    ...
    assert (tmp_path / "heldout_queue_position_latency_edge_survival.csv").exists()
    assert (tmp_path / "queue_position_latency_edge_survival_scorecard.json").exists()
    assert (tmp_path / "heldout_queue_position_latency_edge_survival_scorecard.json").exists()
    ...
    nonlinear_surface_summary = json.loads(
        (tmp_path / "baseline_nonlinear_stress_surface_summary.json").read_text()
    )
    assert {
        "stress_cells",
        "supported_cells",
        "fragile_cells",
        "supported_cell_share",
        "supported_row_share",
        "weighted_rmse_lift_vs_core",
        "worst_cell_lift",
        "high_stress_cell",
        "high_stress_lift",
        "publishable",
