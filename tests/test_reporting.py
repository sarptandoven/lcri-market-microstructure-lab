...
def test_verify_event_level_passive_fill_horizon_sweep_requires_event_source(tmp_path) -> None:
    pd.DataFrame(
        [
            {
                "horizon": 0.5,
    ...
    assert verify_event_level_passive_fill_horizon_sweep(tmp_path) == []

    pd.DataFrame(
        [
            {
                "horizon": 0.5,
                "event_depletion_source": "snapshots",
                "rows": 100,
                "bins": 3,
                "regimes": 2,
                "weighted_mean_predicted_fill_probability": 0.45,
...
def test_verify_artifact_manifest_rejects_non_list_artifacts(tmp_path) -> None:
    manifest = {"artifacts": "metrics.csv"}
